...
@dataclass
class DataCollatorForBartLanguageModeling:
    """
    Data collator used for language modeling. Inputs are dynamically padded to the maximum length of a batch if they
    are not all of the same length.
    ...
    def perm_tokens(self, inputs_id, sep_token=[2,4]): #for bart tokenzier id-> 2: eos, id 4:'.'
        mask = torch.zeros(inputs_id.shape).type(torch.bool)
        for e in sep_token:
        ...
        sep_index_pair = sep_index_pair[torch.randperm(len(sep_index_pair))]
        new_index = torch.zeros(inputs_id.size(),dtype=torch.int32)
        k = 0
        for start, end in sep_index_pair:
            if start == 0:
                sen_len = end - start + 1
                new_index[k:k+sen_len] = torch.linspace(start, end, steps=end-start+1).type(torch.int32)
            else:
                sen_len = end - start
                new_index[k:k+sen_len] = torch.linspace(start+1, end, steps=end-start).type(torch.int32)
            k += sen_len
            
        return inputs_id[new_index.tolist()]
    ...
    def mask_tokens(
        ...
        if self.perm:
            if len(perm_inputs.size())==1:
                perm_inputs.unsqueeze_(0)
            for i in range(perm_inputs.size()[0]): # process the sentence permutation example by example in a batch
                perm_inputs[i] = self.perm_tokens(perm_inputs[i])
        ...
        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool() & masked_indices
        perm_inputs[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)

        # 10% of the time, we replace masked input tokens with random word
        indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
        random_words = torch.randint(len(self.tokenizer), labels.shape, dtype=torch.long)
        perm_inputs[indices_random] = random_words[indices_random]

        # The rest of the time (10% of the time) we keep the masked input tokens unchanged
        return perm_inputs, labels